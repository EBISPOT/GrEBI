            ...
            Iri|Literal => {
                ...
                if EQUIV_PREDICATES.contains(&p_iri.as_str()) {
                        ...
                        let equiv = match p_iri.as_str() {
                            "http://purl.obolibrary.org/obo/chebi/inchi" => { serialize_equivalence(equiv_left, ("inchi:".to_owned()+&o_compact).as_bytes()) },
                            "http://purl.obolibrary.org/obo/chebi/inchikey" => { serialize_equivalence(equiv_left, ("inchikey:".to_owned()+&o_compact).as_bytes()) },
                            "http://purl.obolibrary.org/obo/chebi/smiles" => { serialize_equivalence(equiv_left, ("smiles:".to_owned()+&o_compact).as_bytes()) },
                            _ => { serialize_equivalence(equiv_left, o_compact.as_bytes()) }
                        };
