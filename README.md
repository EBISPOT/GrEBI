...
And so do the Rust dataload crates. The shared library has unit tests, and every pipeline binary has golden tests: the built binary runs on recorded inputs (mostly lifted from the E2E test subgraphs) and its output must match the recorded output byte for byte. The cases live in each crate's `tests/golden/`, and a case can also pin what the binary reports on stderr; the HETT pesticide spreadsheet ingests, the only Python ingests left, have the same kind of cases under `dataload/test_support/python_goldens/`, run with `python3` (which needs pandas and openpyxl). When a change to a binary is intended, rerun with `UPDATE_GOLDEN=1` to rewrite the recorded outputs, then review the diff before committing:
