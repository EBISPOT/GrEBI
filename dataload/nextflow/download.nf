    ...
    for (int i = 0; i < sources.size(); i++) {
        ...
        if (is_url) {
            if (is_tarball && dest_is_dir) {
                ...
                script_lines << "if curl -fSL --retry 5 --retry-all-errors --retry-delay 10 -o \"\$GREBI_TMPZIP\" \"${source}\" && unzip -o \"\$GREBI_TMPZIP\" -d \"${downloads_path}/${dest}\"; then"
                ...
                script_lines << "echo \"Trying source: ${source}\""
                script_lines << "mkdir -p \"${downloads_path}/${dest}\""
                script_lines << "if curl -fSL --retry 5 --retry-all-errors --retry-delay 10 -o \"${downloads_path}/${dest}${filename}\" \"${source}\"; then"
                ...
                script_lines << "if curl -fSL --retry 5 --retry-all-errors --retry-delay 10 -o \"${downloads_path}/${dest}\" \"${source}\"; then"
                script_lines << "    echo \"Success: downloaded ${source}\""
                script_lines << "    exit 0"
                script_lines << "else"
