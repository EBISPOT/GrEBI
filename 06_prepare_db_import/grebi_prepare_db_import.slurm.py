...
def main():
    ...
    print(get_time() + " --- Prepare DB import")
    ...
    input_nodes_filenames = os.path.join(os.environ['GREBI_HPS_TMP'], os.environ['GREBI_CONFIG'], "05_materialise", "nodes_*")
    input_edges_filenames = os.path.join(os.environ['GREBI_HPS_TMP'], os.environ['GREBI_CONFIG'], "05_materialise", "edges_*")
    ...
    our_nodes_file = list(filter(lambda f: int(f.split('/')[-1].split('_')[1].split('.')[0]) == int(task_id), all_nodes_files))[0]
    ...
    if os.environ['GREBI_USE_SLURM'] == "1":
        ...
        if os.system('bash -c "' + cmd + '"') != 0:
            print("moving solr files failed")
            exit(1)
