...
def main():
    ...
    os.makedirs(os.path.join(os.environ['GREBI_NFS_TMP'], os.environ['GREBI_CONFIG'], '05_prepare_db_import'), exist_ok=True)
    ...
    if config['use_slurm'] == True:
        ...
        slurm_cmd = ' '.join([
            'sbatch',
            '--wait',
            '-o ' + os.path.abspath(os.path.join(os.environ['GREBI_NFS_TMP'], os.environ['GREBI_CONFIG'], '05_prepare_db_import', 'prepare_db_import_%a.log')),
            ...
            './05_prepare_db_import/grebi_prepare_db_import.slurm.sh',
            config_filename
        ])
        if os.system(slurm_cmd) != 0:
            print("prepare_db_import failed")
            exit(1)
        os.system("tail -n +1 " + os.path.abspath(os.path.join(os.environ['GREBI_NFS_TMP'], os.environ['GREBI_CONFIG'], '05_prepare_db_import', '*.log')))
    else:
